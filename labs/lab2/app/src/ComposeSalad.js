...
function ComposeSalad() {
  ...
  let foundations = Object.entries(inventory).filter(
    ([name, props]) => props.foundation,
  );
  let proteins = Object.entries(inventory).filter(
    ([name, props]) => props.protein,
  );
  let extras = Object.entries(inventory).filter(([name, props]) => props.extra);
  let dressings = Object.entries(inventory).filter(
    ([name, props]) => props.dressing,
  );
